...
import { 
  ...
  MessageSquare, Trophy, FileText, Globe, BookOpen, Building,
  Crown
} from 'lucide-react';
        ...
        <Card className="relative overflow-hidden border-borders">
          ...
          <CardContent className="relative z-10 p-8 flex flex-col justify-center h-full">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-bold tracking-widest uppercase text-primary-text">
                {displayUser.team ? 'Clan Honor' : 'Total Score'}
              </p>
              {displayUser.team && (
                <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full bg-japan-red/10 text-japan-red border border-japan-red/20">
            ...
            <div className="flex items-baseline gap-2 mb-4">
              <span className="font-serif text-5xl font-bold text-japan-red">
                {(displayUser.team?.score ?? displayUser.score) || 0}
              </span>
            ...
            <p className="text-sm font-semibold text-primary-text mb-4 flex items-center gap-2">
              {displayUser.team ? (
                <>
                  Fighting under <span className="font-bold text-japan-red">{displayUser.team.name}</span>! <NinjaStarIcon className="w-3.5 h-3.5 text-primary-text opacity-80" />
                </>
              ) : (
                <>
        ...
        <CardContent className="p-0">
          <div className="flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-borders">
            ...
            <div className="flex-1 p-5 lg:p-6 flex items-center gap-4 hover:bg-white/50 transition-colors">
              <div className="p-3 bg-white border border-borders rounded-full shadow-sm">
                {displayUser.team ? <Shield className="h-5 w-5 lg:h-6 lg:w-6 text-japan-red" /> : <Shield className="h-5 w-5 lg:h-6 lg:w-6 text-primary-text" />}
              </div>
              <div>
                <p className="text-[10px] font-bold tracking-widest uppercase text-muted-text mb-1">
                  {displayUser.team ? 'Clan Role' : 'Role'}
                </p>
                <p className="font-bold capitalize text-primary-text">
                  {displayUser.team ? (displayUser.teamRole === 'leader' ? 'Clan Leader' : 'Clan Member') : (displayUser.role || 'User')}
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 p-5 lg:p-6 flex items-center gap-4 hover:bg-white/50 transition-colors">
              <div className="p-3 bg-white border border-borders rounded-full shadow-sm">
                <Trophy className="h-5 w-5 lg:h-6 lg:w-6 text-japan-red" />
              </div>
              <div>
                <p className="text-[10px] font-bold tracking-widest uppercase text-muted-text mb-1">
                  {displayUser.team ? 'Realm Rank' : 'Rank'}
                </p>
                <p className="font-bold text-primary-text">
                  {displayUser.team?.rank && displayUser.team.rank > 0
                    ? `#${displayUser.team.rank} (${displayUser.rank || 'Clan'})`
                    : (displayUser.rank || 'Ronin')}
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 p-5 lg:p-6 flex items-center gap-4 hover:bg-white/50 transition-colors">
              ...
              <div>
                <p className="text-[10px] font-bold tracking-widest uppercase text-muted-text mb-1">
                  {displayUser.team ? 'Clan Honor' : 'Score'}
                </p>
                <p className="font-bold text-primary-text">
                  {(displayUser.team?.score ?? displayUser.score) || 0} pts
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-[#FAF7F2] border-borders relative overflow-hidden">
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-4 z-10 relative">
              <CardTitle className="flex items-center gap-2 text-lg font-serif">
                <Users className="h-5 w-5 text-japan-red" /> {displayUser.team ? 'Clan & Warriors Roster' : 'Clan Status'}
              </CardTitle>
              <Button 
                variant="outline" 
                size="sm" 
                className="bg-white text-japan-red border-japan-red hover:bg-japan-red/5 shadow-sm h-8 font-bold"
                onClick={() => router.push('/leaderboard')}
              >
                <Trophy className="h-3.5 w-3.5 mr-1" /> Clan Standings
              </Button>
            </CardHeader>
            <CardContent className="z-10 relative">
              {displayUser.team ? (
                <div className="space-y-4">
                  {/* Clan Summary Header */}
                  <div className="p-3.5 rounded-lg bg-white border border-borders shadow-xs flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      <div className="p-2 rounded-md bg-japan-red/10 text-japan-red border border-japan-red/20">
                        <Shield className="h-4 w-4" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-base text-primary-text">{displayUser.team.name}</h4>
                          <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-secondary-bg text-secondary-text border border-borders">
                            {displayUser.team.teamType || `${displayUser.team.memberCount || 1} warriors`}
                          </span>
                        </div>
                        <p className="text-xs text-muted-text flex items-center gap-2 mt-0.5">
                          <span>Your Role:</span>
                          {displayUser.teamRole === 'leader' ? (
                            <span className="font-bold text-amber-700 inline-flex items-center gap-1">
                              <Crown className="w-3 h-3 fill-amber-500 text-amber-500" /> Leader
                            </span>
                          ) : (
                            <span className="font-semibold text-primary-text">Member</span>
                          )}
                          {displayUser.teamJoinedAt && (
                            <>
                              <span>•</span>
                              <span>Joined {new Date(displayUser.teamJoinedAt).toLocaleDateString()}</span>
                            </>
                          )}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-text">Realm Rank</p>
                        <p className="text-base font-bold text-japan-red">
                          {displayUser.team.rank && displayUser.team.rank > 0 ? `#${displayUser.team.rank}` : '—'}
                        </p>
                      </div>
                      <div className="h-7 w-px bg-borders" />
                      <div className="text-right">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-muted-text">Clan Honor</p>
                        <p className="text-base font-bold text-primary-text">{displayUser.team.score || 0} pts</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-text px-1">
                      Clan Comrades ({displayUser.team.members?.length || displayUser.team.memberCount || 1})
                    </p>
                    
                    {displayUser.team.members && displayUser.team.members.length > 0 ? (
                      <div className="space-y-2">
                        {displayUser.team.members.map((member) => (
                          <div
                            key={member.id || member.userId || member.username}
                            className="flex items-center justify-between p-3 rounded-lg border border-borders bg-white shadow-xs hover:border-japan-red/30 transition-colors"
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <Avatar className="h-9 w-9 border border-borders shrink-0">
                                <AvatarImage src={member.avatarUrl || `https://avatar.vercel.sh/${member.username}`} />
                                <AvatarFallback className="text-xs font-bold font-serif">
                                  {member.username?.slice(0, 2).toUpperCase()}
                                </AvatarFallback>
                              </Avatar>
                              <div className="min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="font-bold text-sm text-primary-text truncate">
                                  ...
                                  {member.teamRole === 'leader' ? (
                                    ...
                                    <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-secondary-bg text-secondary-text border border-borders">
                                      Member
                                    </span>
                                  )}
                                  {member.rank && (
                                    <RankBadge rank={member.rank as Rank} size="sm" />
                                  )}
                                </div>
                                <div className="flex items-center gap-2 mt-0.5 text-xs text-muted-text">
                                  <span>@{member.username}</span>
                                  {member.score !== undefined && (
                                    <>
                                      <span>•</span>
                                      <span className="font-semibold text-primary-text">{member.score} pts</span>
                                    </>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              {member.username && (
                                <a
                                  ...
                                  title={`View @${member.username} on GitHub`}
                                  className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-md bg-secondary-bg hover:bg-neutral-200 text-primary-text border border-borders transition-colors"
                                >
                      ...
                      <div className="p-4 rounded-lg bg-white border border-borders text-center">
                        <p className="text-xs text-muted-text">Roster is being gathered by scouts...</p>
                      </div>
                    )}
                  </div>
                </div>
              ) : (
                <div className="py-6 px-4 rounded-lg bg-white border border-borders text-center flex flex-col items-center">
                  <div className="w-12 h-12 rounded-full bg-secondary-bg flex items-center justify-center mb-3">
                    <User className="h-6 w-6 text-muted-text" />
                  </div>
                  <h4 className="font-bold text-primary-text mb-1">Solo Warrior (Ronin Path)</h4>
                  <p className="text-xs text-secondary-text max-w-sm mb-4 leading-relaxed">
                    You are walking the solitary path of mastery. All honor points and realm rankings represent your individual triumph.
                  </p>
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="font-bold text-xs bg-white border-borders hover:border-japan-red hover:text-japan-red"
                    onClick={() => router.push('/leaderboard')}
                  >
                    <Trophy className="h-3.5 w-3.5 mr-1" /> View Leaderboard
                  </Button>
                </div>
              )}
            </CardContent>
                  ...
                  <div className="grid grid-cols-3 gap-4 w-full p-4">
                     ...
                     {badges.map((badge) => (
                         <div key={badge.id} className="flex flex-col items-center text-center gap-2">
                           <Image src={badge.imageUrl || '/images/bamboo.png'} alt={badge.name} width={48} height={48} className="w-12 h-12 object-contain" />
